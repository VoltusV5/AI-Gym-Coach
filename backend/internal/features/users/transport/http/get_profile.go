...
func (h *UsersHTTPHandler) GetProfile(rw http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	profile, err := h.usersService.GetProfile(ctx, userID)
	if err != nil {
		responseHandler.ErrorResponse(err, "failed to get profile")
		return
	}
